...
function setupSocketListeners() {
  ...
  socket.on('card-status', async (data) => {
    ...
    try {
      const res = await fetch(`${BACKEND_URL}/card/${data.uid}`);

      if (currentUser.role === 'agent') {
        // Agent: update topup and register sections
        ...
        if (res.ok) {
          ...
          isNewCard = false;
          if (topupUid) topupUid.value = data.uid;
          ...
          // Existing card scanned on register = show message
          if (regUid) regUid.value = data.uid;
        ...
        // Salesperson: update card details, checkout
